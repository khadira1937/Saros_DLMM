...
export type Result<T> =
  | { ok: true; value: T }
  | { ok: false; error: DomainError; detail?: string };
...
const mockPools: readonly PoolRef[] = [
  {
...
const mapValidationError = (error: ZodError): Result<never> => ({
  ok: false,
  error: 'InvalidInput',
  detail: summarizeIssues(error),
});

const mapError = (error: unknown): { error: DomainError; detail?: string } => {
...
const inferMode = (rpcUrl: string | undefined): MODE => {
  if (!rpcUrl) {
    return MODE.DEVNET;
  }
...
let cachedClient: LiquidityBookServices | null = null;
let cachedKey: string | null = null;

const getClient = (rpcUrl: string): LiquidityBookServices => {
...
const deriveSeed = (wallet: string): number => {
  let total = 0;
...
const mockPositions = (wallet: string): UserPosition[] => {
...
const toPoolRef = (metadata: PoolMetadata): PoolRef => ({
...
const computeMidPriceFromMetadata = (metadata: PoolMetadata): number => {
...
export async function listPools(): Promise<Result<PoolRef[]>> {
  if (isMockMode()) {
  ...
  return wrap(async () => {
    ...
    const metadataList = await Promise.all(
      addresses.map((address) => client.fetchPoolMetadata(address))
    );
  ...
  return wrap(async () => {
    ...
    for (const pool of poolsResult.value) {
      ...
      for (const raw of rawPositions ?? []) {
        const binLower = Number((raw as { lowerBinId: number }).lowerBinId ?? 0);
        const binUpper = Number((raw as { upperBinId: number }).upperBinId ?? 0);
        const positionAddress = (raw as { position: string }).position;
        let amountBase = '0';
        let amountQuote = '0';
        ...
        if (positionAddress) {
          ...
          const totals = reserveInfo.reduce(
            (acc, reserve) => {
...
export async function priceToBinIndex(
  pool: string,
  targetPrice: number
...
export async function currentMidPrice(
  pool: string
): Promise<Result<number>> {
